...
const StoryRing: React.FC<StoryRingProps> = ({ stories }) => {
  ...
  const scrollSpeedRef = useRef<number>(0.015); // Default desktop speed
  ...
  useEffect(() => {
    const updateScrollSpeed = () => {
      if (window.innerWidth <= 768) {
        scrollSpeedRef.current = 0.80; // Mobile speed
      } else {
        scrollSpeedRef.current = 0.015; // Desktop speed
      }
    };
  ...
  // Calculate container width and single loop width
  const getStoryWidth = () => {
    return window.innerWidth <= 768 ? 112 : 128; // Mobile: 96px image + 16px gap, Desktop: 112px image + 16px gap
  };

  const getSingleLoopWidth = () => {
    return sortedStories.length * getStoryWidth(); // Width of one full set of stories
  };

  const getContainerWidth = () => {
    return getSingleLoopWidth() * 3; // Triple the stories for seamless looping
  };
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      setPosition(prev => {
        ...
        const maxPosition = containerWidth / window.innerWidth * 100; // Max position in percentage
        const newPos = prev + scrollSpeedRef.current * (delta / 16);

        // Seamless loop: reset to start of second loop when reaching the end of the second loop
  ...
  const handleDragMove = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    const sensitivity = window.innerWidth <= 768 ? 0.7 : 0.5; // Higher sensitivity for mobile
    const singleLoopWidth = getSingleLoopWidth();
    const newPosition = startPositionRef.current - (diffX * sensitivity) / window.innerWidth * 100;

    // Keep position within bounds and loop seamlessly
    setPosition(prev => {
  ...
  const getDefaultTitle = (lang: string) => {
    return lang === 'tr' ? "2025-2026 YENİ SEZON FORMALAR İÇİN TIKLA" : "CLICK FOR 2025-2026 NEW SEASON JERSEYS";
  };
  ...
  return (
    <div className="w-full py-6 overflow-hidden">
      ...
      >
        ...
        >
          {/* Render stories three times for seamless looping */}
          {[...sortedStories, ...sortedStories, ...sortedStories].map((story, index) => (
            <div 
              key={`${story.id}-${index}`} 
              className="flex flex-col items-center cursor-pointer"
              style={{ transition: 'all 0.3s ease-out', minWidth: window.innerWidth <= 768 ? '96px' : '112px' }}
            ...
            >
              <div 
                className={`
                  ...
                  ${hoveredStoryId === story.id ? 'scale-125 z-50' : 'scale-100'} 
              ...
              <span className={`
                ...
                ${hoveredStoryId === story.id ? 'text-foreground' : 'text-gray-800 dark:text-gray-200'}
              `}>
                {language === 'tr' 
                  ? (story.title_tr || getDefaultTitle('tr')) 
                  : (story.title_en || getDefaultTitle('en'))}
              </span>
            </div>
          ))}
        </div>
