...
const Footer: React.FC = () => {
  ...
  const renderLogo = () => {
    if (!settings) return <span className="text-2xl font-bold">FormaYaptirma</span>;
    
    if (settings.logo_type === 'image' && settings.logo_image_url) {
      return <img src={settings.logo_image_url} alt="Logo" className="h-8" />;
    }
    
    const logoText = settings.logo_text || 'FormaYaptirma';
    const logoTextParts = logoText.split(/(?=Yaptirma)/);
    
    return (
      <span className="text-2xl font-bold font-poppins">
        {logoTextParts[0]}
        <span className="text-primary">{logoTextParts.length > 1 ? logoTextParts[1] : ''}</span>
      </span>
    );
  };
